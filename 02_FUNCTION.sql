...
-- Oracle 에서는 INSTR() 함수
-- 찾을 문자열의 위치를 반환 (1부터 시작, 못 찾으면 0 출력)
...
-- employees 테이블에서 부서코드가 DEV 인 사원의 수
-- 1) JOIN - ON
SELECT COUNT(*)
FROM employees E
...
-- 결과: World MySQL (마지막 World 만 MySQL 로 변경됨)
SELECT REGEXP_REPLACE('World World', 'World$', 'MySQL');
